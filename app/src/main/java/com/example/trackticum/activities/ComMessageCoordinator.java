...
public class ComMessageCoordinator extends AppCompatActivity {
    ...
    String comId, coordinatorId, coordinatorName;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_com_message_coordinator);
        setupWindowInsets();
    ...
    private void initializeData() {
        ...
        comId = sharedPreferences.getString("com_id", null);
        ...
        adapter = new CoordinatorMessageAdapter(this, messageList, comId);
    ...
    private void sendMessage(String message) {
        ...
        String url = Constants.API_BASE_URL + "/company/send-message";

        StringRequest stringRequest = new StringRequest(Request.Method.POST, url,
                response -> {
                    progressDialog.dismiss();
                    try {
                        JSONObject jsonResponse = new JSONObject(response); // Parse response as JSON
                        if (jsonResponse.getBoolean("result")) { // Check if 'result' is true
                            String messge = jsonResponse.getString("message"); // Extract 'message'
                            Toast.makeText(this, messge, Toast.LENGTH_LONG).show(); // Display message
                        } else {
    ...
    private void fetchMessage() {
        String url = Constants.API_BASE_URL + "/company/get-coordinator-messages/" + comId + "/" + coordinatorId;
                ...
                response -> {
                    ...
                    for (int i = 0; i < response.length(); i++) {
                        try {
                            ...
                            String receiverId = comId; // Assuming this is the company ID
    ...
    private void messageEventListener() {
        ...
        Channel channel = pusher.subscribe("chat.company-" + comId + "-user-" + coordinatorId);
